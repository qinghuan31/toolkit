...
### 新增功能
- **全局历史功能块解耦**：`peel_data_extraction_history` 表新增 `plugin` 列，全局历史页从 `PluginManager` 拉取已注册插件并显示为「功能块」下拉过滤；批次标题、摘要卡、明细表全部展示所属功能块，让历史真正按"跨所有功能块统一聚合"的产品定位工作
- **历史表索引扩展**：新增 `idx_..._plugin` 索引；`request_id` 与 `operation_time` 索引保留，确保大批量历史下聚合查询和明细查询走索引而非全表扫描

### 功能改进
- **历史页加载性能优化**：页面构造改为 `QTimer.singleShot(0, refresh)` 延迟刷新，避免同步阻塞切换；批次明细按需加载并按 `request_id` 缓存；表格批量填充时 `setUpdatesEnabled(False)`，减少大表渲染时的 UI 抖动
- **历史页 UI 增强**：顶部新增「功能块」下拉框（包含「全部功能块」+ 已注册插件 +「数据提取（兜底）」），切换功能块会自动重查数据并清空明细缓存；批次列表标题、摘要、详情统一带 `[plugin]` 前缀，跨功能块历史可一眼区分

### 问题修复
- 修复"全局历史"页只能看到 `peel_data` 一个功能块记录的问题，根因是历史表无 `plugin` 维度字段，读取侧也无筛选入口
- 修复历史页首屏同步刷新导致点击侧边栏后页面卡顿的问题（典型 200+ 行明细批次）
- 修复历史表无索引导致聚合查询走全表扫描的问题（旧库在 `ensure_history_table()` 启动时自动补索引）
...
### 新增功能
- **首次使用引导**：应用首次打开时弹出模式选择页，用户可在进入主界面前选择「单机使用 / 作为主机 / 连接主机」
- **可跳过引导**：首次引导顶部和底部均提供固定位置「跳过」入口，跳过后默认进入单机模式，后续仍可在「综合设置」中切换
- **首次价值说明**：首屏使用 10 字以内价值文案「快速汇总剥离数据」，并只引导「选模式 → 选目录 → 一键提取」核心路径

### 功能改进
- **主界面文案优化**：侧边栏「剥离数据汇总」简化为「数据提取」，减少专业术语，新用户更容易理解入口含义
- **数据提取页引导优化**：页面顶部新增三步路径提示「选择文件夹 → 开始提取 → 检查导出」，将关键操作前置
- **操作按钮更易懂**：「选择目录」改为「选择文件夹」，「开始提取」改为「开始提取并预览」，「保存到数据库」改为「提取后保存结果」
- **状态提示优化**：初始状态从「就绪」改为「先选择文件夹，再开始提取」，降低首次使用空白感
- **启动信息同步**：QApplication 版本号改为读取 config 单一来源，避免应用元信息继续停留在旧版本

### 问题修复
- 修复首次启动缺少明确模式选择导致用户不清楚 local/server/client 区别的问题
- 修复主界面首屏缺少核心任务路径提示，用户需要自行理解操作顺序的问题
